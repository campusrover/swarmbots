...
def follow():
    ...
    # if wall ahead, try to get unstuck with robust wandering first
    if wall_ahead:
        return wander()
...
def wander():
    ...
    if min(g_ranges['F'], g_ranges['FL'], g_ranges['FR']) < TOO_CLOSE_TO_TURN:
        ...
        vel_msg.linear.x = MAX_LINEAR_VEL
    elif g_ranges['F'] < MIN_WALL_DIST: # check if anything in front is too close (will collide if continue moving)
